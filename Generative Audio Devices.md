...
- **T7a — Perceptual parameter vocabulary.** Two phases. **Phase 1 (closed 2026-04-21)** — numeric value emission: the emitter now honors PDL `*` lines and writes parsed numbers or registry defaults into `.vcv`, closing the audible-test automation gap. **Phase 2 (open)** — registry extension: each param carries perceptual regions and curve hints (linear / log / exp); PDL accepts named regions as values (`CUTOFF = dark`); same register of work as T4b and T11, a schema commitment. Phase 2 intercepts the non-finite-value branch phase 1 already surfaces as a warning — named regions resolve instead of falling back to default. Closes the "every preset sounds neutral" problem without archetypes, because the generator can say `CUTOFF = bright` instead of leaving the default.
...
**Suggested execution order:** T7a phase 2 → T7b → T6 refactor → T10. Phase 1 of T7a (numeric emission) is closed; phase 2 (regions + curves) is the remaining schema commitment and stays scoped and fast. T7b is the big new piece; T6 re-derives after both exist; T10 becomes the first real test of cross-target archetype portability.
