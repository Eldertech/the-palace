...
**Remaining tail (human, Mac-side):** the Rack audition — generate `# archetype: kick #seed=1`, `# archetype: warm_pad #seed=1`, `# archetype: pluck #seed=1`, load all three, confirm *"at least three archetypes are perceptually distinguishable on first load without any manual knob adjustment."* Fixtures and the determinism proof are staged in `Artifacts/Shop/VCV Patch Generator/` via the Specialist's first Piece-tier job.
