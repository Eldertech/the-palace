...
The Review Layer and [[Enrichment]] are two halves of one loop. Enrichment *sends* — a page made rich, its sound and interactives laid beside its words. The Review Layer *collects* — Loudon's reactions, in context, structured. Every rich face carries its own review surface, cut to this entry's granularity rule: one note per section of the page, plus one for the page as a whole, sent to [[STIGMERGY]] as a `human_eval` from TRICKSTER — the loudon-eval poster, generalized past the rating sheet. What comes back feeds the page's next round.
