...
| **Sentry** | A pre-push hook + secret scanner / security linter | A data-loss-prevention scan before publish | An access-control audit of what the graph exposes | A proofread for what must not be printed | A review of what the Published Language exposes |
...
| **Deposit** | "Let's deposit" / "Add this to the palace" | A find worth adding to canon, from any door — the session, a harvest, a close, a steward | Re-enter → consult the [[Concierge]] → careful map → Loudon reads every word → check → commit as `deposit(<id>):` with the record in the commit body | The find crossed and the commit self-classifies on the LOG deck (`Palace-Kind: deposit`), or it was declined with a reason; the placement is verified, not asserted; whoever brought it was answered |
...
| **Sentry** | "sentry sweep" / "security sweep" | The repository is public, or a step is about to widen what it shows | Sweep; raise masked findings to Loudon; hold open ones locally; refuse a credential at the push | Every raised finding held and put to Loudon; the ledger has the run's line; no tracked file names an open finding | [[Sentry]] |
...
| **Enrichment** | "enrich [page]" / "enrich this page" / "let's enrich [page]" | One entry, read whole with its bundle | The whole Shop makes the page's rich face — sound, image and interactives beside its headings, via a manifest in the bundle; pieces test the text, and what making teaches goes home shown first, or through the Deposit map when it changes what the page says | Every manifest section *in step*; the manifest and its pieces in the bundle, so the face switch offers the rich face; the page renders at `/rich/` | [[Enrichment]] |
