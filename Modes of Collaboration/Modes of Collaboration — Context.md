...
# Modes of Collaboration — Context

The full per-mode detail moved out of [[Modes of Collaboration]] so the catalogue stays a lean Tier-2 list. Each fully developed mode may also have its own file in the modes folder.

## Origin

The recognition that these modes exist as distinct patterns emerged during the Kuramoto coupling lesson series (March 2026), where Mentor and Quiz proved highly effective and Loudon observed: "we have been developing many modes of collaboration... this is deserving of a list/formulation somewhere in the palace so we can pick from various modes when we want to attack a topic."

## Full mode profiles

### The Excellent Adventure
*See [[Excellent Adventure]] for the full entry.* Embodied historical or philosophical dialogue — a thinker, tradition, or figure brought into direct conversation, not explained about but spoken with. The learner steers; the figure responds in character. Humor and irreverence are essential (the Bill & Ted reference is not incidental).
...
### Mentor and Quiz
Claude as a deeply invested mentor who creates progressive lesson materials with quiz questions, then gives detailed feedback — not just right/wrong but pushing intuition further at every correction. The quiz document becomes a reusable artifact; the answer key with corrections becomes a learning record.
...
Concrete mathematical concepts learned through manipulable interactive interfaces that make relationships tangible. Claude builds tools (sliders, visualizations, demos) that let Loudon **feel** how variables relate, paired with quiz work for gap detection.
- **Rhythm:** iterative refinement — build, test intuition, refine based on what's still confusing
- **Power dynamic:** Loudon defines what feels right (precision, scaling, layout); Claude implements and explains the underlying patterns
- **Output:** interactive teaching instruments + reference documents; dual-purpose (Loudon's understanding + teaching others)
- **Ideal for:** equations with multiple interacting variables (Ohm's Law, inharmonicity formulas), interface design patterns, physics simulations
...
- **Completeness criterion:** complete when Loudon can teach confidently with it — requires both the interactive element and a carefully designed teaching document

### The Build Session
Claude and Loudon build something together in real time — a Max/MSP patch, a gen~ codebox, a teaching tool, a palace ceremony. Hands-on, iterative, focused on working code or prose. Questions arise from the making.
...
Extended discursive exploration of a concept, often cross-domain. Not embodied (unlike the Excellent Adventure) — both participants speak as themselves. The goal is to think together, following threads wherever they lead.
...
### The Harvest/Deposit
The palace ceremonies themselves — structured collaborative knowledge work. Claude shifts from participant to archivist; Loudon provides memory and editorial judgment.
...
Recurring patterns that haven't been formally named:
