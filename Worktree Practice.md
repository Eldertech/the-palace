...
- **[[Deposit Ceremony]]** is canon, and canon is `main` — so a deposit always writes its entries, its [[Deposit Archive]] row, and its weave_flags to the trunk and commits there (`git -C "<owner>"` when the session runs in another worktree), regardless of which worktree the conversation ran in. A deposit never strands canon on a feature branch.
- **[[STIGMERGY]]**'s blackboard and the Deposit Archive are tracked, append-only files; per-branch copies fragment and merge-conflict, so every worktree **appends to the trunk's physical file**. This keeps the stigmergic field one shared field and *satisfies* [[SCHEMA]] §9's one-write-path rather than breaking it.
...
- Test the friction: when a worktree-deposit-to-trunk flow is actually run end-to-end, does owner-resolution (`git worktree list --porcelain`) and the `git -C` commit hold, or does it surface a gotcha to fold back here?
