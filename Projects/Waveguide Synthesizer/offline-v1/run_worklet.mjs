...
// Every 'wave' message the page posts is also kept with its extra fields
// (view, capture instant, phase, frame number) in <vis.f32>.wave.f32 and
// <vis.f32>.wave.json, and the last 'replay' recording in <vis.f32>.replay.f32
// + .replay.json — added in cycle 9 to measure the strobe and replay views.
...
let Proc = null;
const visFrames = [];
const waveFrames = [], waveMeta = [], replays = [];
let now = 0;
class AudioWorkletProcessor {
  constructor() {
    this.port = { onmessage: null, postMessage: (d) => {
      if (d.type === 'replay') { replays.push(d); return; }
      if (d.type !== 'wave') return;
      if (!score.probeEvery) visFrames.push(d.data);
      const { data, ...meta } = d; waveFrames.push(data); waveMeta.push({ ...meta, postedAt: now });
    } };
  }
}
...
if (visPath) {
  ...
  const w = new Float32Array(waveFrames.length * 180);
  waveFrames.forEach((f, i) => w.set(f, i * 180));
  fs.writeFileSync(visPath + '.wave.f32', Buffer.from(w.buffer));
  fs.writeFileSync(visPath + '.wave.json', JSON.stringify(waveMeta));
  if (replays.length) {       // the last replay recording the page handed over
    const r = replays[replays.length - 1];
    fs.writeFileSync(visPath + '.replay.f32', Buffer.from(r.data.buffer));
    const { data, ...meta } = r;
    fs.writeFileSync(visPath + '.replay.json', JSON.stringify(meta));
  }
}
