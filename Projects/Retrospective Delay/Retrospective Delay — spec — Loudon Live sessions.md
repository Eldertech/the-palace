---
title: Retrospective Delay — spec — Loudon Live sessions
born: 2026-09-25
links:
...
**Reveal (10 min):** Participants play into the device. The moment of recognition — hearing the whole phrase come back as a monolithic block, not as echoes. Not "echo echo echo." A ghost.
...
### Prerequisites
Basic Max/MSP fluency: `phasor~`, `buffer~`, `plugin~`/`plugout~`. Participants who need the M4L Audio Effect format can use:
...
### Materials
- `stage-1/stage-1-the-witness.md` — the session draft (arc, framing, reveal, hilaritas checklist, open questions), drafted by the steward on 2026-05-27.
- [[Retrospective Delay — spec — stage-1-patch]] — patch spec (object list, connections, key calculations, defaults, three verification checks).
- [[Retrospective Delay — spec — stage-1-mockup-imagery]] — Shop-routed brief for the five lesson mockup assets (Maker → p5.js, Matplotlib, Remotion, Midjourney). Spec'd, not dispatched.

---

## The séance
*The second move · vanilla Max, performance-ready*
...
**Performance (15 min):** Live improvisation. Loudon performs first, showing the core technique: play a phrase, let silence trigger the summoning, layer new material over the ghost. Then participants play.
...
### Prerequisites
The witness complete. The gain knob and lag parameterization build directly on its patch.

### Materials
- *To be developed after the witness is delivered*
...
**Make (45 min):** Port the DSP core into a `gen~` subpatcher. `phasor` works identically. `buffer`, `poke`, `peek` all exist at sample rate. The structure maps 1:1 — but participants feel the difference in how they think about what they're doing. Every object in the gen~ patcher is a sample-rate operation. There is no ambiguity.
...
### Prerequisites
The witness and the séance complete — the Gen~ migration only lands if participants built the vanilla version first. Participants who need Gen~ orientation can use:
...
### Materials
- *To be developed after the séance is delivered*

---

## The face
*The fourth move · an animated JSUI interface*
...
**Make (45 min):** Build the JSUI drawing code. Three character states mapped to gain value ranges. Participants choose their own character — Loudon's cat/spiritualist guide is the example, not the requirement. JSUI approach (real-time vector drawing) rather than extracted animation frames, for portability and teachability.
...
## The portal
*The fifth move · RNBO → VST/AU and H90 export*
*~90 minutes | Making/Framing: 50/50*
...
**Make (45 min):** Port Gen~ → RNBO codebox~. The signal-rate way of thinking is already established in the grammar session — so this migration is conceptually smoother than that one. Syntax differs but architecture is identical. Export VST/AU. Export to H90.
...
### Prerequisites
The grammar complete. Basic familiarity with RNBO helpful — participants who need orientation can use:
...
### Materials
- *To be developed after the face is delivered*
