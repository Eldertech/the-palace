...
That identity is what makes pitch-class space **circular** rather than linear: C4 and C5 occupy the same angular position on the circle, just at different radii (registers). Octave equivalence is the foundation of that geometry. The Shepard tone exploits the circularity directly — distributing energy across octave-related partials and fading them at the spectral boundaries lets ascent loop back on itself without the ear ever registering a seam.

### Stage 1 — A Static Drone Is Already an Illusion

Pair with Stage 1 above (the **MINIMUM-ILLUSION** static drone, implemented at `Projects/Shepard Tone Synthesizer/shepard_synth.py`).
...
This is the claim the static drone makes: octave equivalence is **strong enough to fuse seven simultaneous sines into one perceived note**. It's the perceptual axiom Stage 2 exploits when it adds discrete pitch-class motion (ASCENT-FIRST) — without this fusion, the staircase illusion would just sound like seven sines getting shuffled.
...
### Where the Theory Still Has an Edge

- **Inharmonic timbres.** In stretched-partial instruments (see [[Piano String Inharmonicity]]), octave "sameness" becomes approximate rather than exact — a real perceptual tension. This is also where the Stage 2 wrap stops being free: in a stretched-octave tuning the dropped voice no longer lands on its own pitch class, and the seam should begin to show by ear.
- **Logarithmic perception.** Octave equivalence is why pitch is logarithmic, not linear — see [[Logarithmic Interface Scaling]].
- **Microtuning and other cycles.** The Shepard effect as built relies on octave equivalence under equal temperament. Other tuning systems, or other interval cycles (fifths, thirds), would need their own equivalence relation to loop on — open territory (see Open Questions above).
