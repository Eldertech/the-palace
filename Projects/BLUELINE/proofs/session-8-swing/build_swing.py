...
RENDER = arg("--render","out/s8_swing.png"); SAVE = arg("--save","")
os.makedirs(os.path.dirname(RENDER) or ".", exist_ok=True)
rng = np.random.default_rng(8)

# ── scene geometry (figure faces +X toward the column) ────────────────────────
...
PIVOT   = Vector((0.12, 0.00, 1.40))          # ~upper chest/shoulder; the rotation centre of a chop
R_GRIP  = 0.52                                 # pivot→hands (arm reach)
R_TIP   = 1.42                                 # pivot→blade tip (arm + blade)
T_IMPACT= 0.25                                 # seconds windup→impact (a fast chop)
FPS     = 24
ACCEL_K = 2.5                                  # θ(u)=Δθ·u^K  → ω peaks at impact (loading→snap)
d_wind  = Vector((-0.34, 0.0, 0.94)).normalized()   # blade points up-and-back at the top of the windup
d_imp   = (CONTACT - PIVOT).normalized()             # blade points to the contact at impact
ROT_AX  = d_wind.cross(d_imp).normalized()           # the swing's rotation axis (≈ world Y)
...
print(f"[s8] DERIVED PHYSICS  Δθ={math.degrees(DTHETA):.0f}°  ω_impact={omega(1.0):.1f} rad/s"
      f"  tip_speed={omega(1.0)*R_TIP:.1f} m/s  |v_contact|={V_CONTACT.length:.1f} m/s"
      f"  dir={tuple(round(v,2) for v in V_CONTACT.normalized())}")
...
if rig:
    ...
    HT=Vector((0.62,0.0,1.00))                          # follow-through: hands extended forward-low
    ...
    for b,ang in (("torso",24),("chest",15)):
        pb=rig.pose.bones.get(b)
        if pb: pb.rotation_mode='XYZ'; pb.rotation_euler=Euler((math.radians(ang),0,0),'XYZ')
...
bm=bmesh.new(); bmesh.ops.create_cone(bm,cap_ends=True,segments=22,radius1=0.16,radius2=0.16,depth=2.2)
...
for i in range(NSH):
    ...
    pos = CONTACT + Vector(tuple(rng.normal(0,0.04,3))) + cone*(sp*DT_STILL*rng.uniform(0.6,1.7))   # framed displacement
...
# ── 6. SPEED-LINES — the blade's DERIVED recent arc; length ∝ |v| (amplified) ─
cu=bpy.data.curves.new("s8_speed",'CURVE'); cu.dimensions='3D'; cu.bevel_depth=0.014; cu.bevel_resolution=2; cu.use_fill_caps=True
back=-vc_dir                                              # the blade came IN from here (−v_contact)
LEN=0.5*GAIN_STREAK                                       # streak length ∝ speed (amplified)
for off in (-0.07,-0.03,0.01,0.05,0.09):
    ts=np.linspace(0,1,10); sp=cu.splines.new('POLY'); sp.points.add(len(ts)-1)
    for i,t in enumerate(ts):
        pp=CONTACT + back*(t*LEN) + perp*off + OUTN*0.04   # sit just off the wound so they read
        sp.points[i].co=(*pp,1); sp.points[i].radius=max(0.03,(1-t)**1.5)   # thick at impact, taper away
...
if cam:
    cam.location=Vector((-1.15,1.25,1.95)); look=(CONTACT+GRIP)/2+Vector((0,0,-0.05))
