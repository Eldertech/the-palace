...
# Optional face (N-keypoint) — draw_facepose (dots), backward-compatible.
nface = 0
face = data.get("face")
if face:
    face_kps = [Keypoint(x, y) if vis else None for (x, y, vis) in face]
    if any(k is not None for k in face_kps):
        canvas = draw_facepose(canvas, face_kps); nface = sum(1 for k in face_kps if k)

Image.fromarray(canvas).save(os.path.join(pose_dir, "openpose.png"))
print("wrote", os.path.join(pose_dir, "openpose.png"),
      "| body:", sum(1 for k in keypoints if k), "/18 | hands:", nhands, "| face pts:", nface)
