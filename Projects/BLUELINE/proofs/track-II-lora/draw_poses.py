#!/usr/bin/env python3
"""Draw the character-sheet openpose skeletons from keypoint JSONs (comfy venv, PIL)."""
import os, json, glob
...
for kpf in sorted(glob.glob(os.path.join(P,"*_keypoints.json"))):
    ...
    for idx,(a,b) in enumerate(d["limbs"]):
        if kp[a][2]>0.4 and kp[b][2]>0.4: dr.line([kp[a][0],kp[a][1],kp[b][0],kp[b][1]],fill=COLORS[idx%18],width=8)
    for i,(x,y,c) in kp.items():
        if c>0.4: dr.ellipse([x-6,y-6,x+6,y+6],fill=COLORS[i%18])
    img.save(kpf.replace("_keypoints.json","_openpose.png")); n+=1
print(f"DREW {n} poses")
