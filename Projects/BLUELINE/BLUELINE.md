...
> **▸ WORK-IN-PROGRESS — on a feature branch, not here.** The latest BLUELINE work lives on **worktree `feature/blueline-m3`** → `/Users/loudonstearns/Documents/palace-feature-blueline-m3`, not merged to main; this entry's "Where it stands" is *more current on the branch*. Canon produced so far — [[Steer the Generator]] and [[Frame Designer]] (incl. the 2026-06-22 + 2026-06-23 field notes) — is on main. **As of 2026-06-23 the 6-shot new-story opening storyboard is COMPLETE** (rich-first / stylize-last pen-flow pipeline validated; RunPod backend built). **Next rung: the animatic** — sequence the boards in time, pinned to a song Loudon brings; **the animatic substrate landed 2026-06-23** (the live-clock-loop, `feature/blueline-m4l` → main `07a67f3`+`236a1b6`: a track-agnostic marker/locator transport + per-track clip scanners, track-name-namespaced with auto-rescan on Stop, + a multi-track previz of every scanner track) — so the rung now needs only the song. The live baton is `Projects/BLUELINE/BLUELINE — baton.md` *on the branch*. Recreate the worktree if gone: `node _ops/worktree/new-worktree.mjs --name feature/blueline-m3 --profile blueline --memory`. Delete this callout when the branch merges.
