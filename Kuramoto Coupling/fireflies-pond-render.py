"""
Fireflies Pond — ComfyUI render driver, Kuramoto Coupling Track A

Loads a ComfyUI workflow JSON, queues it against a locally-running
ComfyUI server, polls for completion, and copies the rendered PNG into
the bundle as `fireflies-pond.png`. The workflow itself
(fireflies-pond-workflow.json) is the reproducibility artifact —
re-runnable byte-identically given the same checkpoint, seed, and
ComfyUI version.

Dispatched by Maker.  Specialist: ComfyUI.  Tier: Sketch (first job).
Project: Kuramoto Coupling.

Prerequisites:
  - ComfyUI cloned at _tools/ComfyUI/ with its venv populated.
  - sd_xl_base_1.0.safetensors in _tools/ComfyUI/models/checkpoints/.
  - ComfyUI server running on 127.0.0.1:8188 (start via
    `python _tools/ComfyUI/main.py --listen 127.0.0.1 --port 8188`).
"""
...
WORKFLOW = BUNDLE / "fireflies-pond-workflow.json"
OUT = BUNDLE / "fireflies-pond.png"
REPORT = BUNDLE / "fireflies-pond.report.json"
COMFY_OUTPUT_DIR = Path(__file__).parent.parent / "_tools" / "ComfyUI" / "output"
