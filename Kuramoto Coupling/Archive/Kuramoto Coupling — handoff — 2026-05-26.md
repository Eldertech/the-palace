...
- **Commits / FIRST ACTION**: clear stale git locks before anything else — `rm -f .git/HEAD.lock .git/index.lock`. The Cowork session committed the Matplotlib credit (`9221e1a`) but the sandbox couldn't unlink the lock files afterward (macOS-side `.git` permissions), so git is currently wedged: **this handoff file and the Active Handoff edit to `Kuramoto Coupling.md` are written to disk but uncommitted.** After clearing the locks, `git add` + commit them, then proceed. Do all commits Mac-side from now on.
