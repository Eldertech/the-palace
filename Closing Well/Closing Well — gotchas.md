...
## From a live close — 2026-07-04 (BLUELINE Aftermath-Frame-tool build session)

11. **A hand-on baton that continues *feature work* means DO NOT tear down its worktree.** The close reflex leans toward cleanup — "tear down the worktree" is even a stock backstage check (gotcha #9). But when the hand-on row is a baton living on that worktree's branch (`_ops/worktree/SKILL.md`: feature-work batons live in the worktree's bundle on its branch), tearing the worktree down strands the very work being handed on. Loudon had to interject "do not tear down this worktree" mid-close. **The rule: if a baton row points at a feature branch, the worktree is part of the handoff — keep it standing, and say so in the baton's receiving-environment coordinate.** Teardown is only for a close whose worktree work fully landed on `main`.
12. **A bare-token assent to a multi-part gate is ambiguous — state your reading before executing.** Loudon signed with "1" to a three-part gate (deposit? renders? move-anything?). "1" most naturally meant "gate 1, first option (deposit: none)" *and* "take your defaults on the rest," but it could have meant only the first. The fix that kept the close honest: **state the interpretation explicitly and give a halt window** ("reading that as X — halt me if wrong") *before* committing, rather than silently proceeding on a guess. The one gate is a signature; a signature you had to interpret should be read back.
