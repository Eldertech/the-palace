...
- **curator** — *built.* Say **"concierge, tidy the links around what I just touched"** → it tends the entries a session touched (`do / offer / flag`): reads the *whole* palace and the web to verify (a check on host hallucination), performs reversible mechanical fixes, and drafts canon changes for your yes. Writes by **graduated consent** — nothing is walled off from being *proposed*; taste or distance from the work only raises the bar from act to ask.
