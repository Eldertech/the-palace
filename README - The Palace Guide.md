...
Any entry may carry `Foo — scroll.md`, its **front door**: a Now zone regenerated on every look, the **Plan** agreed with Loudon (changed only with his yes, every change logged on the trail), Loudon's Standing Orders, and an append-only trail of what it has made ([[The Scroll]]). A ceremony also keeps `Foo — tuning.md`, the ledger of its runs — one line each — and of what they changed. A move's Loudon Live session design, when one is made, is a `spec` the plan links to. Under the **Machinery/Content Split** ([[Project Stewardship System]]) the engine's runtime files (manifest, state, history) stay in `_ops/` — content lives with the entry, machinery stays in ops.
