...
## Forward Vectors
- **Hands now, face next.** The `default` rig already carries **38 finger bones** (so `draw_handpose` from the 21-point hand is almost free) and eye/jaw bones; the full 70-point face comes from fixed mesh-landmark verts (`draw_facepose`), and the xinsir ControlNet already reads body+hands+face. Plan + trade-offs: [[Figure Rig — face-hands-openpose design]].
