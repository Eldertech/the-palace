...
**2026-09-25 — The SA3 model card holds three things this entry didn't.** Re-checked on the Hub (`stabilityai/stable-audio-3-small-music`). **Inpainting:** SA3 supports targeted edits and continuing a short recording; `stable-audio-tools` exposes it as `generate_diffusion_cond_inpaint`. Untested by us. **A second licence layer:** besides the Stability AI Community License, the T5Gemma text encoder (`google/t5gemma-b-b-ul2`) is redistributed under the Gemma Terms of Use, use restrictions in §3.2 included, accepted at the gated download; commercial use points to https://stability.ai/license. That sharpens the open question about monetized Loudon Live use. **Lighter routes exist:** `stabilityai/stable-audio-3-optimized` (Stability's own), MLX ports (`mlx-community/stable-audio-3-small-music`, `-small-sfx`), `Comfy-Org/stable-audio-3` for ComfyUI, and a community ONNX build (`lsb/stable-audio-3-small-music-onnx`) — all confirmed to exist, none run here. The card's "a few seconds on a MacBook Pro M4" agrees with our 2026-05-26 timings. *(Proposed by the [[Shopkeeper]]; verified and landed 2026-09-25.)*
