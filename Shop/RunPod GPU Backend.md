...
- **Raise the readiness timeout for FLUX cold-pulls — it's a gate, not just a slow queue.** A pod orchestrator's `wait_ready` that defaults to ~12 min can give up *before* a fresh FLUX worker image finishes its cold pull + first model load, then tear the pod down and retry forever — burning money on boots that never get a job. Bump `boot_timeout` to ~30 min (720→1800s on the m3 orchestrator), and **amortize the boot by rendering many jobs per pod**, not one-pod-per-image. When a host is just slow, retry on a *fresh* host rather than recycling the same one mid-pull.
- **An interrupted orchestrator leaks the pod — always sweep after a killed run.** The orchestrator terminates its pod in a `finally:` block. If the *process itself* is killed — session teardown, a `TaskStop`, a crashed shell — that `finally` never runs, and the pod keeps **RUNNING and billing** with nothing watching it. This bit us live: a torn-down run left pod `x1izjb30p8jwrt` (and a sibling) running until I swept the API. **Rule: any time a RunPod background orchestrator is killed or interrupted, immediately list pods (`GET /v2/pods`), find anything still RUNNING, and terminate it.** Don't trust the `finally`; trust the sweep. Belt-and-suspenders: have the orchestrator write its pod id to a known file on launch, so a sweep can target it even without the API list. **Multi-agent caveat (see next bullet): scope the sweep to your OWN namespaced pod name — a blind `GET /pods → terminate all RUNNING` will kill another agent's pod.**
- **The account is shared — namespace every pod, or two agents strangle each other.** The most expensive lesson of all ([[assume multi-agent]]): on 2026-07-02 a full day looked like a RunPod outage — ~10 pods booted `RUNNING` but ComfyUI never came up. Real cause: a *second* Claude on the same account. Every orchestrator was single-tenant — one shared pod name (`blueline-sdxl-pose-cn`), a startup guard that aborted if *any* pod existed, and name-based cleanup/cull that deleted *by name* — so each agent's guard and sweeps silently terminated the *other's* still-booting pod. A pod killed at ~120s reads exactly like a slow boot that never finishes. **Fix (shipped 2026-07-02):** a single-sourced per-agent slug at `_ops/runpod/agent_ns.py` (`$RUNPOD_AGENT_SLUG` → git worktree name → host). Pod name is now `blueline-…-<slug>`; the guard, cleanup, cull, terminate and pod-id file (`/tmp/pod_id-<slug>`) are all scoped to that name, so an agent only ever sees and touches its own pods. Optional cooperative capacity lease over the STIGMERGY board via `RUNPOD_LEASE=1` (`_ops/runpod/gpu_lease.py`; advisory — `RUNPOD_LEASE_STRICT=1` to serialize). Proof: `python3 _ops/runpod/test_multi_agent.py` (14 checks, two slugged agents can't see each other's pods). Serverless-endpoint tools (`regen_one.py`, `make_faces.py`, `serverless-client.py`) are inherently multi-agent-safe — an endpoint is shared managed infra, not a single-tenant pod — and need no slug.
- **A create call can leak even when it "fails" — check state, don't retry blind.** RunPod's `POST /v1/pods` can return a non-200 (flaky HTTP 500) while *still* creating a running pod. A retry loop that reads 500 as "no capacity, retry" leaks one pod per retry — this spawned **11 running pods** (~$6/hr) on 2026-06-23 before a guard caught it. Fix: after any non-200, *recover the pod by name* (`GET /pods`, match name) instead of retrying; cull extras (keep one, delete the rest) on both success and recovery; only retry if *no* pod exists. General rule for any "create" API whose side effect can outlive an error: **the error code is not proof the side effect didn't happen** — check actual state by name/id before retrying.
- **A clean exit is not proof the account is clean — reap your namespace after every run.** A run that completed and whose `finally` fired (DELETE→404, "confirmed gone") still left a *second* pod running and billing: the flaky create had spawned a sibling that `_cull_extras` missed (it culls at create-time; the sibling surfaced on a later racy retry), and the orchestrator only ever tracked its own recorded id. **Rule: after every pod run, success or failure, reap YOUR-namespaced pods** — `commons reap --self --force` (`_ops/commons/reaper.py`, same-slug only). Never `GET /pods → DELETE all` or delete by the bare shared name; that kills other agents' pods. Also: pods can **boot-then-die** (RUNNING for ~2 min then the container exits, ComfyUI never serving — a CUDA-host init crash); RUNNING ≠ ready.
- **Every RunPod API call needs a browser user-agent, not just the pod proxy.** The WAF that 403s Python's default user-agent also guards `rest.runpod.io` (endpoint scaling) and the serverless API. On 2026-09-24 a face batch died at its first `PATCH` with a bare 403, which reads like a bad key; the key was fine (a GET with a browser UA returned 200). `regen_one.py`, `serverless-client.py` and `_ops/commons/endpoint.py` now send one. Diagnose any RunPod 403 with a read-only GET, default UA against browser UA, before suspecting credentials.
...
- **Serverless endpoint wiring.** The key + endpoint id live in the **Shop** (`RunPod Images/studio/config.json`, gitignored), *not* in `_ops/commons`. Endpoint `iy3ybd7qjl2trj` is the palace-FLUX ComfyUI serverless worker (`flux1-dev-fp8.safetensors`); SDXL can ride it only if its models are staged on the shared volume (unconfirmed — the launcher only uses FLUX). Drive it via `serverless-client.py` → `RunPodEndpoint(endpoint_id, api_key).run({"workflow": <comfy API json>})`. Two macOS framework-Python (3.14) gotchas: HTTPS to `api.runpod.ai` dies `CERTIFICATE_VERIFY_FAILED` unless you `export SSL_CERT_FILE=$(… certifi/cacert.pem)` (serverless-client has no certifi fallback); and loading the hyphenated `serverless-client.py` via `spec_from_file_location` must set `sys.modules[name] = mod` *before* `exec_module`, or py3.14's `@dataclass` throws. `workersMax` is auto-scaled by the Commons **`EndpointWorkers`** (`_ops/commons/endpoint.py`), ref-counted across agents — no manual dashboard scale-up. For pen-flow **SDXL** work the ready substrate is **local `_tools/ComfyUI`** (SDXL base + ControlNets staged), not this endpoint.
