...
- **⚠ The pod tooling is SINGLE-TENANT — two Claudes on one RunPod account strangle each other (diagnosed 2026-07-02).** After a full day of phantom "dud nodes" — pods that booted to `RUNNING` but where ComfyUI *never* came up and `wait_ready` timed out at ~575s — the cause was almost certainly a **second Claude terminating this one's pod mid-boot**. Three account-wide collision points in `pose_pod_orchestrator.py`: **(1)** every pod shares one name (`blueline-sdxl-pose-cn`), so `cleanup_named` / `_cull_extras` — which delete *by name* — kill the other agent's pod; **(2)** `main()`'s guard aborts if **any** pod exists on the account (`GET /pods`), so it can't tell "mine" from "theirs"; **(3)** the stray-sweeps (the rule just above, plus the retry loops) do `GET /pods` → **DELETE all**, so each agent's sweep-before-boot terminates the other's still-booting pod. A pod killed at ~120s reads `RUNNING` for a while, never answers on the proxy, and looks exactly like a dud. **Until the tooling is namespaced per-agent, only ONE agent may touch RunPod at a time** — serialize with a TRICKSTER `RESOURCE_REQUEST` GPU lease on the STIGMERGY board. The multi-tenant fix (namespace pod names per agent; scope every list/cull/sweep/guard to the agent's *own* name; per-agent `/tmp/pod_id`) is the real answer — see the standing rule: **[[assume multi-agent]] use in every palace tool from day one.**
