...
- **An interrupted orchestrator leaks the pod — always sweep after a killed run.** The orchestrator terminates its pod in a `finally:` block. If the *process itself* is killed — session teardown, a `TaskStop`, a crashed shell — that `finally` never runs, and the pod keeps **RUNNING and billing** with nothing watching it. This bit us live: a torn-down run left pod `x1izjb30p8jwrt` (and a sibling) running until I swept the API. **Rule: any time a RunPod background orchestrator is killed or interrupted, immediately list pods (`GET /v1/pods`), find anything still RUNNING, and terminate it.** Don't trust the `finally`; trust the sweep. Belt-and-suspenders: have the orchestrator write its pod id to a known file on launch, so a sweep can target it even without the API list.
- **The orchestrator's own teardown can miss a leaked SIBLING — the finally is necessary but not sufficient.** *(2026-07-02.)* A run that *completed normally* (its `finally` fired, `DELETE`→404, "confirmed gone") still left a **second pod `rcu5xnsi913lyc` RUNNING and billing** that the orchestrator never knew about. The flaky create had spawned a sibling that `_cull_extras` didn't catch (it culls at create-time; the sibling appeared on a later racy retry). The recorded pod was torn down; the sibling was not. Only a post-run **`GET /v1/pods` sweep of the whole account** caught it. **Rule: after *every* pod run — success or failure — sweep the account and DELETE anything named `blueline-sdxl-pose-cn`.** A clean orchestrator exit is not proof the account is clean. An independent watchdog that runs `--cleanup` on a timer is the reliable dead-man's switch, since it doesn't depend on the orchestrator's bookkeeping.
- **Pods can boot-then-die: RUNNING ≠ ready.** *(2026-07-02.)* A pod went `RUNNING`, then `desiredStatus` went `None` ~2 min in and ComfyUI never served through the full 30-min `wait_ready` (container init crash — the CUDA-driver-host class). `wait_ready` correctly failed and tore it down, but it burned 30 min of boot. Add a CUDA≥12.6 host filter to the create body and, when a host is dead rather than slow, retry on a *fresh* host rather than waiting out the whole timeout.
