...
Shipped 2026-06-09 (`b2a5ccb`): the mode is live on the [[STIGMERGY]] armed-write path. `checkAllowList` split into `checkPathSafety` (security — repo escape / NUL / non-`.md` / VCS+build trees, *never* relaxed) and `checkCanon` (care — canon files, ceremony cards, `_ops/` machinery — the line the trickster crosses); `previewEdit`/`armedWriteEntry` take a `trickster` flag that gates on path-safety only; the lane's `trickster({ path, op })` commits in one gesture as `Palace-Author: trickster` + `verify: unverified`; `POST /api/entry-agent/trickster` plus the `/trick <text>` gesture in the Companion window land it, and the committed proof's existing `[undo]` *is* "untrick last"; `Palace-Author: trickster` carries a magenta brand in the LOG deck. The server path is verified end-to-end (unit + integration: canon bypass, brand, undo, path-safety refusal); the `/trick` input gesture and the LOG brand are compile-checked, awaiting a live click-through.

**Second surface — the EDIT form** (`fbf03dc`): a real write straight from the STATE entry editor, no Companion at all. Edit the whole entry by hand (frontmatter + body), then a magenta `⚡ trickster commit` button at the bottom lands it immediately — `POST /api/entry/trickster-save` → `tricksterSaveEntry` emits the file and commits as `Palace-Author: trickster` + `verify: unverified`. The editor now opens for canon (its pre-flight is `checkPathSafety`, not the care gate), the verify dropdown is gone (a trickster is always unverified — the brand *is* the verify), and the landed hash carries `[untrick]` (the same honest `/undo` revert). Where the Companion path is one *op* (append/rewrite), this is a whole-file write — the same posture worn over the form. Server path integration-tested (canon write, brand, path-safety refusal); the button/untrick UI is compile-checked, awaiting a live click-through.
