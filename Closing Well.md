...
**Structure verifies cheaply, and it lies.** An artifact that parses, resolves, and validates its own shape can still be behaviourally wrong: a scene collection whose transforms OBS rescales on load, a board line whose invented `dispatch_mode` falls off the validator's allowlist, a render whose arrows never move. Each of those passed every check available to a reader of the file. The check that tells the truth is the one that observes the *running* system — rendered pixels, a live query back from the API, the strict validator itself. So prefer, wherever the choice exists, **an interface you can read back from over a file you can only write to.** That is the transferable half: it is why an application's socket beats its config directory, and it generalises the frozen-arrows incident above from "check your renders" into a rule about where to look. *(Four failures in one 2026-09-02 session, across two media, taught the same lesson twice.)*
