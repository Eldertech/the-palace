...
**The fold-vs-fragment check never ran.** A new entry was minted beside `Loudon Live.md`, which already carried a § describing the same thing. It was the right call in the end, but nobody asked the question — it lived in the [[Closing Well — tuning|gotcha ledger]] as gotcha 18, where a deposit ceremony never reads it. The map now asks it first, and asks it of the [[Concierge]] rather than of the drafter, since minting is the drafter's lower-friction default.
...
**v2.0 (2026-09-25) — the gate for every find.** Tuning item 14. The card had grown around one door, an archivist returning to a past conversation, while finds now arrive from the session in the room, from harvests, from closes and from stewards. Loudon set the shape: a deposit is for larger additions to canon, not upkeep; it stays reflective and slow, with a careful map and the elder consulted, whoever is asking; it is the palace's protection, carrying its values and caring for whoever brings the find. What left the card already lived elsewhere: the entry template in the README, the body-link pass in SCHEMA §4, filing in SCHEMA — Reference §8. The harvest-era invocation paths collapsed into one opening, and the pace obligations became § Care for whoever is bringing it. v1.0's requirement that at least one link carry a label was dropped on purpose: the map asks of every link whether it deserves one, and a label added to meet a count is a forced connection.
