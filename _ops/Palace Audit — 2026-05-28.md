...
### Deferred from pass 3 (need Loudon)
...
- **Schema ratification of `exemplifies` (×50) + `member-of` (×48):** NOT done on autopilot — reverses this audit's own normalization and requires the Schema Change Protocol (SCHEMA §change-protocol) + documented rationale. Ceremony doc to be prepared for an explicit decision. See §"Schema decision" below.
