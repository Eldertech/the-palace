...
export default function LoginScreen({ onLogin }) {
  const [stage, setStage] = useState('dialing'); // dialing -> banner -> login
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      ...
      if (i === 5) {
        clearInterval(t);
        setTimeout(() => setStage('banner'), 220);
      }
    }, 170);
      ...
      {stage !== 'dialing' && (
        <>
          <pre data-testid="login-banner" style={{
            ...
            fontFamily: 'var(--font-mono)', fontSize: 11, lineHeight: 1.05,
            whiteSpace: 'pre', textAlign: 'center',
          }}>{BANNER}</pre>
          <div style={{
            textAlign: 'center', color: 'var(--phosphor-dim)', textShadow: 'none',
          ...
          <div style={{
            ...
            &gt;&gt; DiSRUPTED &amp; CRACKED BY :{' '}
            <span style={{ color: 'var(--ansi-bright-red)', textShadow: '0 0 6px var(--ansi-bright-red)' }}>tRiCKSTER</span>
            {' '}&lt;&lt;
          </div>
          <div style={{
            textAlign: 'center', marginTop: 4, color: 'var(--phosphor-dim)',
            textShadow: 'none', fontSize: 12, letterSpacing: '.08em',
          }}>
