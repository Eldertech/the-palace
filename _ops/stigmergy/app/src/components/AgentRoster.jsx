...
export default function AgentRoster({ agents, activeHandle }) {
  return (
        ...
        {agents.map((a) => {
          ...
          return (
            <div key={a.handle} style={{ display: 'flex', gap: 8, padding: '3px 0', alignItems: 'center' }}>
              <span style={{
                color: a.active ? 'var(--phosphor)' : 'var(--phosphor-dim)',
                textShadow: a.active ? 'var(--glow)' : 'none',
              }}>
                {a.active ? '●' : '○'}
              </span>
              <span style={{ color: 'var(--ansi-bright-cyan)', textShadow: 'var(--glow)' }}>
                @{a.handle}{me && ' (you)'}
              </span>
              {a.thinking && (
                <span style={{
                  marginLeft: 'auto', color: 'var(--phosphor-dim)',
                  textShadow: 'none', fontSize: 11,
                }}>thinking...</span>
              )}
            </div>
          );
