// primitives.jsx — shared BBS atoms, exported to window
const { useState, useEffect, useRef } = React;

function Banner({ children, strong, dim, as = "div", style = {} }) {
  ...
  const Tag = as;
  return <Tag style={{ fontFamily: "var(--font-display)", color, textShadow: glow, textTransform: "uppercase", letterSpacing: ".02em", ...style }}>{children}</Tag>;
}

function Box({ children, title, tone = "double", style = {}, pad = true }) {
...
function Rule({ double = false, children }) {
  const c = double ? "═" : "─";
  return <div style={{ whiteSpace:"pre", color:"var(--phosphor-dim)", textShadow:"none", fontFamily:"var(--font-mono)", lineHeight:1 }}>{children ? `${c.repeat(2)} ${children} ${c.repeat(Math.max(0, 74-(children?.length||0)))}` : c.repeat(78)}</div>;
}

function Button({ children, hot, tone = "default", onClick, disabled }) {
  const toneMap = {
  ...
  return (
    <button
      ...
      style={{
        ...
        border: `1px solid ${disabled ? "var(--fg3)" : toneMap.border}`,
        textShadow: inverted || disabled ? "none" : "var(--glow)",
        fontFamily: "var(--font-mono)", fontSize: 13,
        padding: "3px 10px", cursor: disabled ? "not-allowed" : "pointer",
        textTransform: "uppercase", letterSpacing: ".04em", borderRadius: 0,
      }}
    >
...
function Tag({ children, tone = "default" }) {
  ...
  const c = colors[tone];
  return <span style={{ border: `1px solid ${c}`, color: c, padding:"0 6px", fontSize:11, fontFamily:"var(--font-mono)", letterSpacing:".05em", textTransform:"uppercase", textShadow: tone==="default"?"none":"var(--glow)" }}>{children}</span>;
}

function Field({ prompt = ">", value, onChange, onSubmit, placeholder, password, autoFocus }) {
...
function TypeOn({ text, speed = 18, onDone, className, style }) {
...
Object.assign(window, { Banner, Box, Rule, Button, Tag, Field, Cursor, TypeOn });
