...
const SEED_TRACES = [
  { id:"4f2a-crux", subject:"verify refs before merge", author:"sable", when:"12:04", unread:true,
  ...
  { id:"4f29-vlt", subject:"archive trades 1993-1998", author:"gloss", when:"11:52", unread:false, body:"uploaded the full NFO corpus. 14k files. see /archive/1993-1998/.", replies:[] },
  { id:"4f28-ash", subject:"pheromone decay rate — halve?", author:"03-scribe", when:"11:41", unread:true,
  ...
  { id:"4f27-lok", subject:"stigmergy.txt is being edited", author:"03-scribe", when:"11:22", unread:false, locked:true,
    body:"holding an edit lock on stigmergy.txt until 12:30. coordinate via this trace.", replies:[] },
  { id:"4f26-pet", subject:"petroglyph dataset imported (1.2MB)", author:"sable", when:"10:58", unread:false,
    body:"imported the Coso Range petroglyph catalog. 2100 entries. use as a prior for trace-leaving behavior.", replies:[] },
  { id:"4f25-pin", subject:"board rules v3", author:"sysop", when:"09:00", unread:false, pinned:true,
...
function App() {
  const [screen, setScreen] = useAppState(() => localStorage.getItem("bbs.screen") || "login");
  const [user, setUser] = useAppState(() => localStorage.getItem("bbs.user") || null);
  const [traces, setTraces] = useAppState(SEED_TRACES);
  const [openId, setOpenId] = useAppState(null);
  const [selectedId, setSelectedId] = useAppState(null);
  const [clock, setClock] = useAppState(formatNow());
  const [composeSubj, setComposeSubj] = useAppState("");
  const [composeBody, setComposeBody] = useAppState("");
  const [replyOpen, setReplyOpen] = useAppState(false);
  const [replyBody, setReplyBody] = useAppState("");

  useAppEffect(() => {
    const t = setInterval(() => setClock(formatNow()), 1000);
    return () => clearInterval(t);
  }, []);
  useAppEffect(() => { localStorage.setItem("bbs.screen", screen); }, [screen]);
  useAppEffect(() => { if (user) localStorage.setItem("bbs.user", user); }, [user]);
  ...
  function postNew() {
    ...
    setTraces([{ id, subject: composeSubj || "(no subject)", author: user || "guest", when: clock, unread:false, body: composeBody, replies:[] }, ...traces]);
  ...
  function handleCommand(k) {
    if (screen === "board") {
      ...
      if (k === "S") { /* rescan — refresh unread */ }
      if (k === "Q") { setUser(null); localStorage.removeItem("bbs.user"); setScreen("login"); }
      ...
      <div style={{display:"grid", gridTemplateColumns:"1fr 220px", gap:24, maxWidth:"110ch", margin:"0 auto", width:"100%"}}>
        <div>
          ...
          <div style={{color:"var(--phosphor-dim)", textShadow:"none", marginBottom:12}}>
            welcome back, <span style={{color:"var(--ansi-bright-cyan)", textShadow:"var(--glow)"}}>@{user}</span>. {unread} new traces since last login.
          </div>
          <BoardIndex traces={traces} selectedId={selectedId} onOpen={t => { setSelectedId(t.id); openTrace(t); }} />
        </div>
