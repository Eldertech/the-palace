// Shell.jsx — page frame with status + command bar
const { useState: useStateShell } = React;

function Shell({ children, user, nodeName, clock, unread, onCommand, commands = [], hidePath }) {
  return (
    <div style={{
      minHeight:"100vh", background:"var(--bg)", color:"var(--phosphor)",
      fontFamily:"var(--font-mono)", fontSize:13, display:"flex", flexDirection:"column",
      position:"relative",
    }}>
      {/* scanlines */}
      <div style={{
      ...
      <div style={{
        ...
        <span style={{color:"var(--phosphor-dim)",textShadow:"none"}}>·</span>
        {user ? <><span style={{color:"var(--ansi-bright-cyan)"}}>@{user}</span><span style={{color:"var(--phosphor-dim)",textShadow:"none"}}>·</span></> : null}
        <span>{clock}</span>
        {unread > 0 && <>
          <span style={{color:"var(--phosphor-dim)",textShadow:"none"}}>·</span>
        ...
        <div style={{
          ...
          {commands.map(c => (
            <span key={c.key}
              ...
              style={{ cursor: c.disabled ? "not-allowed" : "pointer", color: c.disabled ? "var(--fg3)" : "var(--phosphor)", textShadow: c.disabled ? "none" : "var(--glow)" }}
              onMouseEnter={e => { if (!c.disabled) { e.currentTarget.style.background="var(--phosphor)"; e.currentTarget.style.color="var(--bg)"; e.currentTarget.style.textShadow="none"; } }}
              onMouseLeave={e => { if (!c.disabled) { e.currentTarget.style.background="transparent"; e.currentTarget.style.color="var(--phosphor)"; e.currentTarget.style.textShadow="var(--glow)"; } }}
            >
