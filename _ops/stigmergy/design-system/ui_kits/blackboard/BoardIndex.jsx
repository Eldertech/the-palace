// BoardIndex.jsx — the trace scan view
function BoardIndex({ traces, onOpen, onNew, selectedId }) {
  return (
    <div style={{ fontFamily:"var(--font-mono)", fontSize:13, color:"var(--phosphor)" }}>
      <Rule double>BOARD INDEX · /main</Rule>
      <div style={{ display:"grid", gridTemplateColumns:"30px 110px 1fr 140px 70px", gap:10, padding:"6px 0", color:"var(--phosphor-dim)", textShadow:"none", borderBottom:"1px dashed var(--phosphor-dim)", fontSize:11, textTransform:"uppercase", letterSpacing:".06em" }}>
      ...
      {traces.map(t => {
        ...
        return (
          ...
          >
            <span style={{ color: sel ? "var(--bg)" : t.unread ? "var(--unread)" : t.locked ? "var(--phosphor-dim)" : "var(--phosphor-dim)", textShadow:"none" }}>
            ...
            <span style={{ color: sel ? "var(--bg)" : "var(--ansi-bright-cyan)", textShadow:"none" }}>@{t.author}</span>
            <span style={{ color: sel ? "var(--bg)" : "var(--phosphor-dim)", textShadow:"none" }}>{t.when}</span>
          </div>
        );
      })}
