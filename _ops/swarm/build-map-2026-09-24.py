...
ceremony. `_ops/` cards without a canonical type stay ops targets (`ops_ghosts`), as before.
    ...
    if rel.parts[0] == "_ops":
        ...
        if len(rel.parts) == 2 or (len(rel.parts) == 3 and rel.parts[1] not in OPS_EXCLUDE):
            ...
            if fm and fm["type"] in CANON_TYPES:
                node_files.append((p, fm))   # a ceremony card with canon type: woven as a node
            else:
                ops_files.append(p)
        continue
    fm = parse_fm(p)
...
print(f"Nodes: {len(node_ids)} ({sum(1 for n in nodes_out if n['ops_card'])} ceremony cards) | Ops targets: {len(ops_ids)} | Edges: {len(edges)}")
