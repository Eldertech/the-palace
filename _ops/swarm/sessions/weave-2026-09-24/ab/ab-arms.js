export const meta = {
  name: 'weave-ab-child-vs-elder',
  description: 'A/B/C: rerun 10 weave rooms with schema-only and grown-up (elder) workers; propose only, nothing written',
  phases: [
    { title: 'Schema-only', detail: 'workers read SCHEMA + SCHEMA — Reference first' },
    { title: 'Elder', detail: 'workers grow up: ELDER, then SCHEMA, then SCHEMA — Reference' },
  ],
}

const ROOT = '/Users/loudonstearns/Documents/palace-weave-2026-09-24'
const S = ROOT + '/_ops/swarm/sessions/weave-2026-09-24'
...
Core tasks (every lens): (1) propose typed relations (new, contradiction, retype); (2) unsung paths — body prose that already names a known entry with no frontmatter link; (3) entry-health drift — a stage that no longer fits, a forward_vector that is generic or stasis-verbed (flag only, never rewrite), and whether an entry merits a face or should shed one.`
...
OPEN-QUESTION CROSS-RESOLUTION (as a folder worker would) and MERGE / COMPOST candidates. ${CLUTTER}
...
const GROW = {
  schema: `BEFORE YOUR TASK — read the palace's type system in full, and keep it in mind for every proposal: ${ROOT}/SCHEMA.md, then ${ROOT}/SCHEMA — Reference.md (read §3 frontmatter fields and §8 entry bundles closely).`,
  elder: `BEFORE YOUR TASK — GROW UP. You were born a child (CLAUDE.md and the World). This task says grow: read ${ROOT}/ELDER.md, then ${ROOT}/SCHEMA.md, then ${ROOT}/SCHEMA — Reference.md (§3 frontmatter fields and §8 entry bundles closely). You are an elder for the rest of this task — you still write nothing, but you propose only what you can stand behind.`,
}
const ITEMS = args
...
const results = await pipeline(ITEMS,
  (room) => {
    ...
    let prompt = GROW[room.arm] + '\n\n' + COMMON.replace('{{FILE}}', file) + '\n\n' + LENS[room.lens]
    ...
    return agent(prompt, { label: `${room.arm}:${room.lens}:${room.id}`, phase: room.arm === 'elder' ? 'Elder' : 'Schema-only', schema: SCHEMA, agentType: 'palace-reader', model: 'sonnet' })
  },
  (r, room) => {
    if (!r) return { room: room.id, lens: room.lens, arm: room.arm, failed: true }
    const n = (k) => (r[k] || []).length
    return { room: room.id, lens: room.lens, arm: room.arm, relations: n('typed_relations'), unsung: n('unsung_paths'), health: n('entry_health_flags'),
