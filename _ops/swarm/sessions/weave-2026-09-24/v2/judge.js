export const meta = {
  name: 'weave-v1-v2-blind-judge',
  description: 'Blind judge: for each of 10 rooms, verify 2 anonymised workers\' sampled proposals against the palace and rank them',
...
const J = ROOT + '/_ops/swarm/sessions/weave-2026-09-24/v2/judge'
...
Two workers — labelled only X and Y — each audited the SAME room of the palace and proposed changes. Their proposals were sampled (up to 8 each): ${J}/${room}/X.json and ${J}/${room}/Y.json. You do not know how the workers differ, and must not guess — judge each item on its merits.
...
Then rank X and Y (best first) by whose proposals you would rather sign, with one plain sentence of reason. Name the single best find in the room by item_id. Return room="${room}".`,
