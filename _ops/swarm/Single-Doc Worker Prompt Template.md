...
4b. NEW ENTRIES THIS CYCLE (born since the last Weave, still under-connected): {{NEW_ENTRIES}}. These are the palace's priority citizens — only a Weave can wire their inbound links, because those links live in established entries like yours. Actively consider whether YOUR entry should link to any of them, and propose those links in `new_entry_links` below. Genuine links only — a real relationship, never one manufactured to hit a number.
5. FACE CHECK — look in this entry's bundle folder ({{ENTRY_NAME}}/) for a hero/avatar: a file named `{{ENTRY_NAME}} — hero.png` and `{{ENTRY_NAME}} — icon.png` (or any `* — hero.png` / `* — icon.png`). If BOTH exist, report `has_hero`/`has_icon` true and stop there. If a face is MISSING and the entry merits one — an always-type (type `project`/`person`/`specialist`/`maker`, a `hub` or ≥5 typed links, or `foundational` stage) or a grey-band judgment (a `philosophy`-pillar or `growing`+ concept, or a `breakthrough`); NEVER for `spore`/`question` or `seed`/`sprout` concepts — propose one in the [[Hero and Avatar Maker]]'s locked hand-drawn art direction. You only PROPOSE prompts — nothing is rendered here (rendering bills and is a gated batch).
...
## OUTPUT — Return ONLY this JSON:
{
  ...
  "new_entry_links": [{"target": "X (new this cycle)", "proposed_type": "...", "proposed_label": "...", "rationale": "why my entry genuinely connects to this newcomer"}],
