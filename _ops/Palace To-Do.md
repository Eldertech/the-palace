...
- [ ] **Decide whether SCHEMA drops out of the auto-loaded floor to Tier 3** — *deliberately deferred 2026-08-25, not dropped.* Post-v1.17 SCHEMA measures 34.7KB (~9.4K tokens), 41% of a ~23.0K-token floor. Loudon's call: it is still long and probably does need breaking up further, but take the task fresh and weigh all the implications — it is not currently causing major problems, and a tiering change reshapes every agent's palace understanding. **Revival condition: the v1.17 changes have been felt across real sessions.** Full framing lives in [[JEWEL]] § Context Loading Tiers and [[SUBSTRATE]] § Open Questions; this line exists so a deliberate pause surfaces on the worklist rather than only in two foundational files.
