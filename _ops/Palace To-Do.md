...
- [ ] **Let the dead-link checker read bundle files** — `_ops/swarm/lint-ghost-links.py` scans only entries: it skips any file whose frontmatter has no canon `type` (`:155-157`), and bundle files carry none by design ([[SCHEMA — Reference]] §8). So a dead `[[link]]` inside a bundle file never reaches the Weave, and as more tools fold into bundles, more of the palace goes dark to it. Widen the scan to bundle files, reported apart from entries so the Weave can tell them apart. First known case: `[[PDL]]` in `Shop/VCV Patch Generator/VCV Patch Generator — spec — PDL generation prompt.md`, which dropped out of the lint's sight when the prompt folded into the bundle (D-2026-09-26-PDLGP, the Concierge's flag); move 4 on the [[Generative Audio Devices]] scroll, a PDL Spec page, would give it a target. Loudon asked for this, 2026-09-26.
