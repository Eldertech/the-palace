...
- **Still open:** Orchestrator v0.2 (v0.1 is **done** — 97 tests; v0.2 = batch cadence, spawn-from-project, scheduled tasks, ~1200–1500 LOC) — the **autonomous scheduler** (auto-fire a cycle without a human click) lives here, the watch-and-steer counterpart to everything above; the rest of the Weave's **posting half** — surfacing the unsung-path emitter as a STIGMERGY trigger (a QUEUE/Weave button over the existing `GET /api/unsung-paths`), and the harder *generative* proposal types (vector-tuning, hub-promotion, mirror sweeps) that need real Weave reasoning, not just detection — plus any further executor ops (`stage_transition`, `label_enrichment`) as the Weave starts emitting them.
