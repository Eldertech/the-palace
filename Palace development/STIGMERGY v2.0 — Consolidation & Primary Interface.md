...
- **Still open:** Orchestrator v0.2 (v0.1 is **done** — 97 tests; v0.2 = batch cadence, spawn-from-project, scheduled tasks, ~1200–1500 LOC) — the **autonomous scheduler** (auto-fire a cycle without a human click) lives here, the watch-and-steer counterpart to everything above; finish wiring Weave *proposals* end-to-end — the **apply-on-grant half landed** with two ops (`set-vector` + `add-link`, above); the remaining half is the Weave (and Deposit) actually **posting structured proposals** that carry an `apply` op (Swarm Weave Phase B), plus any further executor ops (`stage_transition`, `label_enrichment`) as the Weave starts emitting them.
