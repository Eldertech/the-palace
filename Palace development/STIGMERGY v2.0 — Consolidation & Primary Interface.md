...
- **Still open:** extend Launch Interactive to **stewards** + steward-*requested* sessions (a steward posts a "request interactive" mark at a critical moment → surfaces with a launch action); Orchestrator v0.2 (v0.1 is **done** — 97 tests; v0.2 = batch cadence, spawn-from-project, scheduled tasks, ~1200–1500 LOC); wire Weave *proposals* end-to-end — the QUEUE already **displays + grants** `vector_proposal`/`weave_flag`, so the gap is the Weave actually *posting* them (Swarm Weave Phase B) and an executor *applying* a grant.
