...
- **The Machinery/Content Split governs where stewardship state lives** (decided 2026-06-09; see the section above and Bundle-Local Stewardship — Production Plan). Engine → `_ops`; per-entry content → the bundle. `[Entry] — scroll.md` is the work-state read-model and front door (it replaced `plan.md` on 2026-09-23); `[Entry] — staging.md` is the teaching arc; the board stays the append-only log. The orchestrator materializes the scroll from what the steward posts; the steward reads `staging.md` and the scroll's Standing Orders, writes neither, and flags arc-level changes rather than editing the teaching design.
