...
- **Max, Gen~ and RNBO builds wait for Loudon; stewards keep going** (decided 2026-09-25). Those builds need him present, so they happen in live sessions. A steward makes proofs in any tool, writes code that informs the later build, prepares the session, and says it is ready without blocking — the environment ladder in a plan still stands (`_ops/orchestrator/prompts/shared.md` § Max, Gen~ and RNBO need Loudon in the room).
