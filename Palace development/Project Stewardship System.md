...
The agent's posture must vary by the entry's `stage`. The discussion budget is inverse to the stage: more conversation up front, less re-litigation later. AI-drafted seed entries tend to *look* finished — prose flows, plans are structured, format matches palace convention — and that polish hides misalignment. Treating polish as quality is the trap to avoid.
...
- **Sensory deliverables require an audition gate.** Per [[Substrate Skill]] § Stage as Alignment Confidence — render the smallest unit that exercises every parameter, pause for human audition, commit to full batch only after acceptance.
...
- **Every steward cycle ends with at least one TRICKSTER ask** (decided 2026-05-27, surfaced by cycle 9 of the GSL pilot). A steward never stops with "done." If a cycle produces open decisions — a wrinkle awaiting the human's ear, a proposal awaiting greenlight, a next-cycle fork — each is formalized as a `RESOURCE_REQUEST` to TRICKSTER with canonical `{id, label}` options. If the cycle's work landed clean and no decision surfaced, that is *not* a stopping condition; it is a signal that the steward has lost contact with the page's `forward_vector`. The steward re-reads the vector, identifies the most ripe sub-vector, and proposes two or three concrete next moves as a non-blocking RESOURCE_REQUEST. The vector is the engine that keeps proposals coming. Loudon's framing: *"if there are no open questions, that means the project needs to look to their forward vector and propose some next steps."* Decisions go to TRICKSTER; information goes to GENERAL — never bury an open decision in BROADCAST prose. Encoded in [the steward prompt template](.claude/skills/palace-orchestrator/prompts/steward.md) and [the shared posting-discipline section](.claude/skills/palace-orchestrator/prompts/shared.md).
