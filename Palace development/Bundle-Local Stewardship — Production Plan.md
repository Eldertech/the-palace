...
- **Freshness is a dimension the done-test never named** (surfaced 2026-07-03, a month into live operation). The done-test is about *location* — "read cold, never parse JSONL." But `plan.md` is a materialized read-model that only refreshes on a steward cycle, so when a grant lands *after* a steward's last cycle and no cycle follows, the plan freezes showing a decision as Open that the board already Resolved. A reader obeying the done-test then gets *correct location but stale state*. On a live heartbeat this self-heals within one cycle (the reconcile is CQRS working as intended); on a **paused** swarm it persists — as it did 2026-07-03, when the swarm had been idle since 07-01 and 12 of 19 plans showed stale Opens (e.g. `shepard-024`, granted 06-25, still Open in the plan). The real gap is that `verify-plans.js` cannot distinguish *stale-but-self-healing* from *genuinely drifted* — it cries RED on any paused swarm. The candidate fix: teach verify a freshness check — a plan-Open decision that is board-Resolved with `resolved.ts > last_active` is STALE (amber), not FAIL (red); genuine drift stays red. Hold it until the lag actually bites in practice — don't harden a check the heartbeat already heals.
