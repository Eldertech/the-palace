---
title: Media Library
type: meta
pillars:
  - philosophy
  - creation
  - tools
  - practice
born: 2026-03
last_activated: 2026-03
activation_count: 1
stage: growing
confidence: foundational
energy: medium
hook_quality: 6
beauty: 6
who_leads: loudon
links:
  - target: "[[FOUR PILLARS]]"
    type: connects-to
  - target: "[[Spinoza Conatus]]"
    type: connects-to
  - target: "[[Hyperdimensional Prism]]"
    type: connects-to
  - target: "[[SUBSTRATE]]"
    type: enables
---

# Media Library
This is a living index of books, authors, films, and media that seed palace ideas. Each entry notes the status (unread, reading, read, engaged, referenced) and the palace connection — the idea or concept it sparked or deepens.

The palace is rhizomatic. These sources are its external mycelium. By mapping them here, we make visible what grows from each source.

## Books & Philosophical Works

| Title                                          | Creator            | Type             | Status     | Palace Connection                                                                                                                                                                                                                                       |
| ---------------------------------------------- | ------------------ | ---------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| *Gödel, Escher, Bach: An Eternal Golden Braid* | Douglas Hofstadter | Philosophy/Music | Reading    | Self-reference, strange loops, tangled hierarchies. The palace's *structure* is Hofstadterian. How meaning emerges from the web of connections rather than from individual entries.                                                                     |
| *Ethics*                                       | Spinoza            | Philosophy       | Engaged    | Spinoza's conatus (the striving of a thing to persist in being) underlies [[Spinoza Conatus]], [[Quantum Synthesizer]], [[Kuramoto Coupling]]. Relational ontology: you are not a discrete entity but a differential relation to everything around you. |
| *The Timaeus*                                  | Plato              | Philosophy       | Referenced | Geometric construction of matter and the cosmos. Seeds [[Crystal Synthesizer]], [[Wallpaper Groups]], the idea that geometry IS creation.                                                                                                               |
| *The Parmenides*                               | Plato              | Philosophy       | Referenced | The One and The Many. How does multiplicity arise from unity? How do individual voices in a [[Kuramoto Coupling]] system emerge from a single frequency space?                                                                                          |
| *Human Error*                                  | James Reason       | Systems/Safety   | Unread     | Latent errors, accident precursors, swissness of systems design. Pending connection to [[Latent Error]].                                                                                                                                                |
| *The Pleasure of Finding Things Out*           | Richard Feynman    | Essays           | Referenced | Joy in exploration, the delight of not-knowing. Embedded in Loudon's pedagogy across the palace.                                                                                                                                                        |
| *On the Origin of Species*                     | Charles Darwin     | Science          | Referenced | Evolution through variation and selection. Generative constraint: small tweaks compound into radically different forms. Seeds [[FOUR PILLARS]] thinking.                                                                                                |
| *The Critique of Pure Reason*                  | Immanuel Kant      | Philosophy       | Referenced | Categories of understanding. How do we structure the palace itself? What are the atomic units of a knowledge system?                                                                                                                                    |

## People as Sources

| Name | Role | Status | Palace Connection |
|------|------|--------|-------------------|
| Rick Rubin | Producer/Philosopher | Engaged | Creative process, the artist as conduit, constraints as teachers. Embedded in [[FOUR PILLARS]] thinking and pedagogical philosophy. |
| Buckminster Fuller | Inventor/Visionary | Referenced | Geometry, tensegrity, the universe as a comprehensive system. Seeds [[Hyperdimensional Prism]], the idea of synergetic geometry. |
| Gilles Deleuze | Philosopher | Referenced | Relational ontology, the rhizome as model. The palace's *architecture* is Deleuzian. |
| Alan Turing | Mathematician/Computer Scientist | Referenced | Computation, the Church-Turing thesis, what can and cannot be computed. Implicit in all [[DSP Frameworks]] thinking. |
| James Turrell | Artist (Light) | Engaged | Perception of light as material. The aesthetics of presence and absence. Connects to palette thinking in [[Hyperdimensional Prism]] and color as constraint. |
| Claude Debussy | Composer | Referenced | Orchestration as timbre design. The whole-tone scale breaks traditional harmonic constraint. Seeds [[Wallpaper Groups]] thinking about symmetry-breaking. |
| Pauline Oliveros | Composer | Referenced | Deep listening, participatory composition, the audience as co-creator. Embedded in [[FOUR PILLARS]] pedagogy. |
| Éliane Radigue | Synthesist | Referenced | The ARP 2500 as instrument for *discovery*. Slow, meditative exploration of parameter space. Model for hands-on learning. |

## Films & Visual Media

| Title | Creator | Type | Status | Palace Connection |
|-------|---------|------|--------|-------------------|
| *Particles of Attachment* | TBD | Documentary/Visual | Unread | Seeding ideas about molecular/granular structure and connection. Pending. |

---

## How to Use This Library

**For research:** Cross-reference a palace entry's "born" date and "pillars" with this library. What was Loudon reading when the entry emerged?

**For idea-seeding:** When stuck on a palace entry, check what sources connect to it. Often a re-engagement with source material sparks new connections.

**For teaching:** Many entries here are touchstones for students. Referencing *GEB*, Spinoza, or Feynman provides context for why the palace is structured as it is.

**For growth:** Entries marked "unread" or "referenced-but-not-engaged" are invitations. Deeper engagement with these sources will likely spawn new palace entries or reveal hidden connections.

---

## Unmapped Territories

Sources referenced in palace entries but not yet catalogued here (to integrate):
- Erik Satie (minimalism, constraint, repetition)
- John Cage (indeterminacy, silence, the prepared piano)
- Laurie Spiegel (algorithmic composition, geometric music)
- Karlheinz Stockhausen (synthesis, serialism, space)
- Merzbow (noise as material)

<!-- CLAUDE → LOUDON: This library is a living stub. Each entry could be expanded with quotes, dates of engagement, and specific passages that seeded palace ideas. Consider maintaining a reading journal that maps directly to palace birth dates. -->

