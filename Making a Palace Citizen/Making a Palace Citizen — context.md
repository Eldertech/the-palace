...
- **Citizens need a retained-memory mechanism.** Enchantment is ephemeral — the agent doesn't remember being enchanted; today only the write-back persists. Loudon wants the *encounters themselves* retained so a citizen can build on them. **The archived Dialectic transcripts are that memory in embryo** — [[Spinoza and Meadows on the Threshold]], [[Eno and Cage on the Hand and the Open Ear]] — durable records a future cast can be loaded with ("here is what you have already been through"). Nothing is lost by holding: Eno's and Cage's proposed self-edits are preserved verbatim in the transcript's Post-dialogue artifacts, ready to apply once each has more retained encounters. This is the concrete shape the open question "what a growth ledger holds" was waiting for.
...
- **Retained-citizen-memory mechanism** *(Loudon, 2026-07-02 — now the gate on self-edits)* — design how a citizen accumulates and *retains* its encounters so self-edits are earned across several, not applied after one. The archived Dialectic transcripts are the raw material; the question is the loading protocol: when casting a citizen again, load its prior transcript(s) as "what you have already been through," and only propose a `forward_vector`/link write-back once the drift is consistent across encounters. Ties directly to the card's open question on the growth-ledger structure. **Concrete first test:** run a *second* encounter for Eno or Cage with its Eno↔Cage transcript loaded, and see whether accumulated retained experience changes (or confirms) the self-edit it proposes.
