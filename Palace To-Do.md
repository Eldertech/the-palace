...
- [ ] **Deposit Ceremony — mid-discussion invocation** — When "let's deposit" is called from within an already rich, well-established conversation, the current ceremony assumes a cold-start context. Needs a dedicated section describing how to orient when the conversation already carries substantial context: what changes about re-entry, what to preserve from the current discussion vs. the source, and how to avoid over-reading palace entries when the necessary context is already present.
