...
| **1 — Skeleton**       | CLAUDE.md + SCHEMA.md                                                                                                        | ~16K          | What can exist, how it's typed — and that the room may hold other agents (SCHEMA §9). |     |
...
Tier 0 + Tier 1 + Tier 2 ≈ **24K tokens** (measured 2026-08-25; ROSETTA is *not* in the auto-loaded floor — it loads on demand as a Tier-3 reference). The remaining context is available for work.

The floor grew from the ~20K this table claimed for months, and the growth is almost entirely one file: **SCHEMA.md is 45% of the floor on its own** (~11K tokens). That is the standing question about this design — SCHEMA is a reference consulted when creating an entry or changing the type system, not something needed to hold a conversation. Whether it stays in the auto-loaded floor, or drops to Tier 3 with CLAUDE.md keeping the trigger to read it, is an open decision, not an oversight.
